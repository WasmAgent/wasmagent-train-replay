...
Only the divergent rank appears in the report, so the output should flag
exactly rank 1 with ``first_divergence_step == 1`` and no entry for the
aligned ranks (0, 2, 3).
"""
...
def print_report(report: DivergenceReport) -> None:
    print(f"  first divergence at step: {report.first_divergence_step}")
    print(f"  similarity by rank: {report.per_rank_similarity}")
    for div in report.divergences:
        b = div.baseline_action
        c = div.candidate_action
        print(f"  rank {div.rank} step {div.first_divergence_step}:")
        print(
            f"    baseline:  type={b.collective_type} mode={b.recording_mode}"
        )
        print(
            f"    candidate: type={c.collective_type} mode={c.recording_mode}"
        )
        print(f"    correlated_collision: {div.correlated_collision}")
        print(f"    correlated_escalation: {div.correlated_escalation}")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(f"  train-replay diff {baseline_dump} {candidate_dump}  # exits 1")
